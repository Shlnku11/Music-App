...
const TrackCard = ({ track, isActive, onPlay }) => {
  const { isPurchased, requestPurchase, checkPurchase } = usePurchase();
  ...
  useEffect(() => {
    if (track.is_premium) checkPurchase(track.external_id);
  }, [track.external_id, track.is_premium]);
  ...
  const handleClick = async () => {
    if (locked) {
      const ok = await requestPurchase(track);
      if (!ok) setInvoiceSent(true);
      return;
    }
    addToHistory(track);
    onPlay();
  ...
  return (
    <div className={`track-card ${isActive ? 'active' : ''} ${locked ? 'locked' : ''}`} onClick={handleClick}>
      <div className="track-cover-wrap">
        ...
        {track.is_premium && !premium.active && !isPurchased(track.external_id) && (
          <span className="premium-badge"><FaStar /> {track.price}</span>
        )}
        {(premium.active || isPurchased(track.external_id)) && track.is_premium && (
      ...
      {invoiceSent && <p className="invoice-hint">Счёт отправлен в Telegram</p>}
    </div>
  );
};
